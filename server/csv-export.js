...
const TABLES = ['users', 'balance_logs', 'pulls', 'shipments', 'login_logs', 'page_views'];

function exportRoute(app, db) {
  const mw = auth(db);

  app.get('/api/admin/export/:table', mw, admin, async (req, res) => {
    try {
      const t = req.params.table;
      if (!TABLES.includes(t)) return res.status(400).json({ error: 'Invalid table' });
      const q = t === 'users'
